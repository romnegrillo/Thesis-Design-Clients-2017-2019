    ...
    def loginButtonClicked(self):
        ...
        if username and password:
            try:
                ...
                message.setWindowTitle("Warning")
                message.setText(
                    "Cannot connect to the cloud. Make sure you have proper internet connection and it is not blocking it.")
                message.setIcon(QtWidgets.QMessageBox.Question)
                message.setStandardButtons(QtWidgets.QMessageBox.Ok)
                message.exec_()
        ...
        try:
            ...
            if choice == QtWidgets.QMessageBox.Ok:
                for i in self.comboBoxList:
                    ...
                    self.symptomsWithValuesToFirebase = dict(
                        zip(self.symptomsNameToFirebase, self.symptomsValueToFirebase))

                print(self.symptomsWithValuesToFirebase)

                if current_user_UID != "":
                    db.child("users").child(current_user_UID).child(
                        "usersymptoms").set(self.symptomsWithValuesToFirebase)
    ...
    def __init__(self, bp_and_heartrate):
        ...
        try:
            ...
            if current_user_UID == "":
                ...
                self.userAge = int(db.child("users").child(
                    current_user_UID).child("userinfo").child("age").get().val())
                self.name = db.child("users").child(current_user_UID).child(
                    "userinfo").child("full_name").get().val()
                self.location = db.child("users").child(current_user_UID).child(
                    "userinfo").child("address").get().val()
                self.gender = db.child("users").child(current_user_UID).child(
                    "userinfo").child("gender").get().val()
                self.doctorNumber = db.child("users").child(current_user_UID).child(
                    "userinfo").child("assigned_doctor_number").get().val()
                ...
                my_stream = db.child("users").child(current_user_UID).child(
                    "assessment").stream(self.stream_handler)
        except Exception as exp:
            print(str(exp))
...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, bp_and_heartrate, isECGActive=False):

        super(ApplicationWindow, self).__init__()

        self.bp_and_heartrate = bp_and_heartrate
        self.isECGActive = isECGActive
        ...
        self.myFig = MyFigureCanvas(isECGActive=self.isECGActive,
                                    x_len=50, y_range=[0, 800], interval=20)
...
class MyFigureCanvas(FigureCanvas, anim.FuncAnimation):
    ...
    def __init__(self, x_len: int, y_range: List, interval: int, isECGActive) -> None:
        '''
        ...
        self.ctr = 0
        self.ecgCalibData = [301, 302, 302, 302, 302, 302, 303, 304, 304, 305, 306, 306, 307, 310, 305, 304, 304, 303,
                             302, 302, 302, 299, 299, 320, 600, 303, 303, 301, 301, 301, 240, 299, 301, 302, 302, 303, 304, 305, 306, 307, ]
    ...
    def _update_canvas_(self, i, y) -> None:
        ...
        if self.isECGActive:
            ...
            y.append(self.ecgCalibData[self.ctr])
        else:
            y.append(400)
