...
# UI file names.
trainingMainFileName = "/capture_main.ui"
trainingSideFileName = "/capture_side.ui"
trainingTopFileName = "/capture_top.ui"

# Directory of the image to save.
trainingSideViewImages = "/capture_sideview_images"
trainingTopViewImages = "/capture_topview_images"

# Textfile names.
trainingSideViewTextFile = "/side_record_capture.txt"
trainingTopViewTextFile = "/top_record_capture.txt"
...
class TrainingSide(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.imageObject = imageprocessing.TrainingImageProcessing(
            usbcam=False, rpicam=True)
    ...
    def captureButtonClicked(self):
        ...
        # Edited removed required valyes for weight and age.
        weight = '1'
        age = '1'
        ...
        if weight and \
            ...
            try:
                ...
                #maxima = self.imageObject.getMaximaEllipse()
                #minima = self.imageObject.getMinimaEllipse()
...
##                        f.write(toWrite)
                    ...
                    rgbName = fullPathTrainingSideViewImages+"/rgb_" + \
                        str(now).replace(" ", "_").replace(":", "_")+".jpg"
                    binaryName = fullPathTrainingSideViewImages+"/binary_" + \
                        str(now).replace(" ", "_").replace(":", "_")+".jpg"
                    cntsName = fullPathTrainingSideViewImages+"/cnts_" + \
                        str(now).replace(" ", "_").replace(":", "_")+".jpg"
                    ellipseName = fullPathTrainingSideViewImages+"/ellipse_" + \
                        str(now).replace(" ", "_").replace(":", "_")+".jpg"

                    totalArea = self.imageObject.getAreaPercentage()
                    maxima = self.imageObject.getMaximaEllipse()
                    minima = self.imageObject.getMinimaEllipse()
                    ...
                    #self.estimatedWeightTB.setText(str(weightEstimated))

                    self.imageObject.saveImage(rgbName, binaryName, cntsName, ellipseName)
...
##                        f.write(toWrite)
                    ...
                    rgbName = fullPathTrainingTopViewImages+"/rgb_" + \
                        str(now).replace(" ", "_").replace(":", "_")+".jpg"
                    binaryName = fullPathTrainingTopViewImages+"/binary_" + \
                        str(now).replace(" ", "_").replace(":", "_")+".jpg"
                    cntsName = fullPathTrainingTopViewImages+"/cnts_" + \
                        str(now).replace(" ", "_").replace(":", "_")+".jpg"
                    ellipseName = fullPathTrainingTopViewImages+"/ellipse_" + \
                        str(now).replace(" ", "_").replace(":", "_")+".jpg"
