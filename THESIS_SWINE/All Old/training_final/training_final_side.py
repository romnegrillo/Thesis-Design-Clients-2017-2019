...
basePath = "./training_sideview_images/"
...
for inputImage in imageList:
    ...
    if len(cnts):
        ...
        cv2.ellipse(maskBinaryEllipse, cv2.fitEllipse(cnts[maxIdxCnts]), (0, 0, 255), 3)
        ...
        cv2.imwrite(outputPath+str(inputImage)[:len(inputImage)-4]+"_binary.jpg", maskBinary)
        cv2.imwrite(outputPath+str(inputImage)[:len(inputImage)-4]+"_cnts.jpg", maskCnts)
        cv2.imwrite(outputPath+str(inputImage)
                    [:len(inputImage)-4]+"_ellipse.jpg", maskBinaryEllipse)

        toWrite = str(inputImage)
        toWrite += ","
        toWrite += str(numWhitePixels)
        toWrite += ","
        toWrite += str(areaPercentage)
        toWrite += "\n"

        with open("topview_data.txt", "a") as f:
            f.write(toWrite)
