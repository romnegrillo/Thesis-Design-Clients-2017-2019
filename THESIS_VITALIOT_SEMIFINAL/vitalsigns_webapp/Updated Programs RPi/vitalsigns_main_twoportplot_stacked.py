...
##        print(data)
        if "XFLAG" not in data:
            ...
            if(float(latitudeCoordinate)==0):
                latitudeCoordinate="14.590128"
            if(float(longitudeCoordinate)==0):
                longitudeCoordinate="120.978450"
...
try:
    ...
    ecgSerial = serial.Serial(ecgSerialPort, 9600)
...
class ApplicationWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, bp_and_heartrate):
        ...
        self.myFig = MyFigureCanvas(x_len=100, y_range=[200, 700], interval=20)
    ...
    def ecgTimerThread(self):
        while self.timerFlag:
            ...
            if self.ecgTimer>0:
                #print(self.ecgTimer)
                self.timerLabel.setText("Please wait for ECG data: " + str(self.ecgTimer) + "s")
                self.ecgTimer=self.ecgTimer-1
                 
            elif self.timerFlag:
                self.timerFlag = False
                self.timerLabel.setText("ECG Data running...")
                MyFigureCanvas.SetECGReady = staticmethod(MyFigureCanvas.SetECGReady)
                
                MyFigureCanvas.SetECGReady()
    ...
    def _update_canvas_(self, i, y) -> None:
        ...
        try:
            ...
            try:
                ...
                if not ecgEvalDone:
                    #data = sensorSerial.readline().decode()
                    data = ecgSerial.readline().decode()
                    #print(data)
                    self.ecgData = int(data)
     
                    self.stackedECGData.append(self.ecgData)
            ...
            if ecgEvalDone:
                if len(self.stackedECGData)-1!=self.ctr:
                    y.append(self.stackedECGData[self.ctr])
                    print(len(self.stackedECGData))
                    print(self.ctr)
                    self.ctr=self.ctr+1
                     
                elif len(self.stackedECGData)-1==self.ctr:
                    #print("DONE GRAPHING ECG")
                    pass
